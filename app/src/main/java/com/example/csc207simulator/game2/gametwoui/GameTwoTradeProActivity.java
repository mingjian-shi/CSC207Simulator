...
/**
 * This activity allows the player to trade with a Professor.
 */
public class GameTwoTradeProActivity extends SettableActivity {
    /**
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_game_two_trade);
        TextView knowledge = findViewById(R.id.knowledgeint5);
    ...
    /**
     * Check whether the professor has the enough knowledge
     */
    public void checknum() {
        try {
            String num = ((EditText) findViewById(R.id.numtoguess2)).getText().toString();
            int NumtoGuess = Integer.parseInt(num);
            boolean ifEnough = gameTwoManager.checkNumPro(NumtoGuess, player);
            String show;
            if (ifEnough) {
                player.addPerformance(1);
                int result = gameTwoManager.showNumPro(NumtoGuess, player);
